...
static StrPtr latest_result_ptr;
...
struct whisper_params {
    ...
    int32_t step_ms    = 800; //说话不连贯就调大该参数
    int32_t length_ms  = 9000;
    int32_t keep_ms    = 1200;
...
//         std::string* p = latest_result_ptr.load();
//         if (p) {
//             res.set_content(*p, "application/json");
//         } else {
//             res.status = 204;  // No content yet
//         }
//     });

//     std::cout << "[✓] Server running on http://localhost:8080/result" << std::endl;
...
void start_server() {
    ...
    svr.Get("/result", [](const httplib::Request&, httplib::Response& res) {
        ...
        // 原子 load：抓一个快照，期间对象不会被释放
        StrPtr snap = std::atomic_load_explicit(
            &latest_result_ptr,
        ...
        if (l == std::string::npos) text.clear(); else text = text.substr(l, r - l + 1);
    ...
    std::cout << "[✓] Server running on http://0.0.0.0:8080/result\n";
    svr.listen("0.0.0.0", 8080);
}
...
int main(int argc, char ** argv) {
    ...
    audio_async audio(params.length_ms);
        ...
        //期望设备名与capture_id匹配
    // int num_devices = SDL_GetNumAudioDevices(SDL_TRUE); // SDL_TRUE = 输入设备
    // std::cout << "\033[32mnum_devices:\033[0m\n" << num_devices << std::endl;
    // for (int i = 0; i < num_devices; i++) {
    //     const char* name = SDL_GetAudioDeviceName(i, SDL_TRUE);
    //     if (!name) continue;
    //     printf("[%d] %s\n", i, name);
    //     if (strstr(name, target_hw.c_str())) {
    //         printf("Matched target '%s' => capture_id = %d\n", target_hw.c_str(), i);
    //         params.capture_id = i; // 如果找到匹配的设备，更新capture_id
    //     }
    // }

    if (!audio.init(params.capture_id, WHISPER_SAMPLE_RATE, target_hw)) {
    ...
    std::thread(start_server).detach(); // 启动服务器线程，发布推理结果
    std::atomic_store_explicit(//初始化
        &latest_result_ptr,
            ...
            // print result;
            {
                ...
                for (int i = 0; i < n_segments; ++i) {
                    ...
                    if (params.no_timestamps) {
                        ...
                        //latest_result_ptr.store(new std::string(text));
                           // 原子 store：传入新推理结果
